...
Below, notice the additional parameters we give `read.csv()`. We covered `stringsAsFactors` last week: it prevents string variables from being imported as factors and instead defaults to character type. With `na.strings`, we can specify how the `NA` values are coded in our data.
```{r}
library(foreign)
...
df_drop_duration <- df_new %>%
  ...
  mutate(dur = interval(endorse_date, drop_date),
         dur_days = as.duration(dur)/ddays(1)) %>%
