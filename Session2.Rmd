...
```{r}
{plot(density(by_e_s$av_pts[by_e_s$super == 1]),
     col = "red")
lines(density(by_e_s$av_pts[by_e_s$super == 0]))}
```
...
```{r, echo = F, warning = F}
attributes(w39.df$F_PARTY_FINAL)
w39.df <- w39.df %>%
...
## A super quick primer on data merging using base `R` functions
...
Ok, now that we know how this works, lets try this with all variables that are present in both data frames.[^8]

[^8]: Please note that if you were to try this for all variables in this dataframe, `R` throws an error that essentially complains because the factor levels (i.e. haven labels) don't have the same values across variables. This is a know issue (see https://github.com/tidyverse/haven/issues/477) that will hopefully be fixed by the `haven` developing team soon. For now, a workaround would be to change the variable types to factor (and/or character), rather than `haven_labelled`. 

```{r}
df_long <- merged_inner %>%
  pivot_longer(names_to = "variable",
               values_to = "value",
               cols = -QKEY)
  
attributes(merged_inner$POL1DT_W38)
attributes(merged_inner$VTPLAN_W38)
```



OK, it worked but the data frame could be more informative. Let's use our data wrangling skills to recode the `wave` variable. 
```{r}
df_long_small <- merged_inner %>%
...
Transforming the data in this way will result in duplicate rows for each respondent. Whether or not this data format is desirable depends on the intended use for the data. 
```{r}
library(tidyr)
table(names(merged_inner))
