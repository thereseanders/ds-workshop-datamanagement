...
Let's compute the average  and total number of endorsement points that each candidate got. Note that, because there are missing values for the cases in which no endorsement has been made, we need to tell `R` what to do with them. 
```{r}
by_endorsee <- df3 %>%
  group_by(endorsee) %>%
...
Suppose I wanted to know how many endorsers each candidate has. 
